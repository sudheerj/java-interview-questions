...
3. ### How does JVM works
   ...
   1. **Class Loader Subsystem:** The class loaders are responsible to load the class files dynamically into JVM during the runtime. The first class that loaded into the memory is usually the class that contains `main` method.This subsystem performs three main activities.
      ...
      1. **Loading:** As part of this step, the class loader will read the .class file and loads its bytecode into memory. The following important information of the class file will be saved in method area(Metaspace in modern JVMs) of JVM.

         1. The Fully Qualified Class Name(FQCN) of the loaded class.
         2. Immediate parent class.
         3. Variables and methods(i.e, constant pool),modifier information
         4. Whether .class is related to Class, Interface or Enum.

         JVM also creates an object of **Class** type(available from **java.lang** package) in the heap memory to represent that class at runtime. But it will be created only on the very first time when the class file is loaded into JVM. This object is helpful for the developers to get class level information.

         There are three built-in classloaders available in Java:

           1. **Bootstrap ClassLoader:** This is a root class loader and typically represented as null. It loads the core java libraries such as **java.base** (`java.lang`, `java.util` etc). This class loader is written in native code(C/C++) unlike other class loaders.

            2. **Platform ClassLoader:** This class loader is responsible for loading platform classes such as Java SE platform APIs, their implementation classes and JDK-specific run-time classes. For example, the platform classes such as `java.net.http`, `java.sql`, `java.xml`.
        ...
        The class loaders follow a **Parent Delegation Model**. When a class is requested, the Application ClassLoader first delegates the request to its parent (Platform ClassLoader), which further delegates to the Bootstrap ClassLoader. If the parent cannot find the class, then the child class loader attempts to load it.
      ...
      2. **Linking:** Linking is the process of taking a class or interface and combining it into the runtime state of the Java virtual machine, preparing it for execution. It involves the following steps:
         ...
         1. **Verification:** This phase ensure the correctness of `.class` file by checking whether the file is properly formatted or not, and ensures it follows JVM bytecode specifications. If the validation is failed, you will receive `java.lang.VerifyError` through **ByteCodeVerifier**.

         2. **Preparation:** In this step, JVM allocates memory for all static variables within classes/interfaces and assign them with default values(0, null, false, etc.).

         3. **Resolution:** In this step, symbolic references in the runtime constant pool are replaced with direct references to classes, methods, or fields. This process may occur at linking time or lazily at runtime, and is part of dynamic linking..

      3. **Initialization:** This is the final phase of class loading. During this phase, static variables are assigned their actual values as defined in the class, and static blocks are executed. The JVM executes a special method called **<clinit>()** to perform this initialization.
   ...
   2. **Runtime data area/ memory area:**
      ...
      1. **Method:** The method area stores below class level information,
         ...
         4. Method data such as name, return type, modifiers, attributes, parameter types etc of each method.
      ...
      3. **Stack:** Each thread which is created in JVM, a separate runtime-stack is also created at the same time. The local variables, method calls and intermediate results are stored in this stack area. But if the thread requires a larger stack size than what is available, JVM throws `java.lang.StackOverflowError` error.
        ...
        For each method call, a stack frame is created in the stack memory area and it is going to be destroyed upon method execution gets completed. Each stack frame is further divided into three sections.
         ...
         3. **Frame data:** This area holds all symbolic references related to constant pool resolution and method returned data. It is also used to store a reference to the exception table(part of method metadata) which has all the information related to catch block in case of exceptions.
   ...
   3. **Execution engine:** The execution engine is responsible for executing the bytecode that is loaded into the JVM. It communicates with various runtime data memories of JVM during the execution. Once the program is finished executing, the memory in the runtime data area will be released.
      ...
      3. **Intermediate Code Generator:** It generates intermediate code.
      4. **Code Optimizer:** It optimizes the intermediate code for better performance.
      5. **Target Code Generator:** It converts intermediate code into native machine code.
      6. **Profiler:** This is used to identify Hotspots(i.e, repeatedly executing methods) and replace hotspots with respective native code.

      7. **Garbage Collector:** Garbage Collector(GC) is responsible to collect and remove unreferenced objects from the heap area. This memory management feature is used to reclaim the unused memory and makes free space for newer objects. This happens in two phases:
        ...
        **Note:**Modern JVMs use more sophisticated algorithms (e.g., generational GC, parallel GC, G1GC) to optimize garbage collection.
...
9. ### What is the importance of hashCode() and equals() methods
   ...
   **1. equals:** The `equals()` method is used to compare equality(i.e, logical equality) of two Objects. By default their implementation compares the identities(i.e, memory addresses) of the object. The Object class defined `equals()` method as follows,
   ...
   There are some rules need to be followed to use equals method:
      ...
      5. **Non-null comparison:** For any non-null object x, `x.equals(null)` should return `false`.

   **2. hashCode:** The `hashCode()` method returns the integer hash code value of the object, used by hash-based collections. This method determines bucket placement. It must be overridden in every class which overrides `equals()` method. The general contract of hashCode is:
   ...
   **Note:** equals() defines logical equality, and hashCode() ensures equal objects are stored and retrieved correctly in hash-based collections.
